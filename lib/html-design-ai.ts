...
⚠ BACKGROUND CONTINUITY: a copy line and the image slot that illustrates it are often split into two separate <section>s (each CONTENT line/slot gets its own section). If a section carrying .s-dark or .s-alt is immediately followed by a section that is a pure continuation of the same beat — an image slot (and/or a short accent-bar/i-divider) with NO new heading or new standalone copy of its own — give that following section the SAME .s-dark/.s-alt class too, so the background doesn't visibly cut between the copy and its image. Stop carrying the class forward as soon as a section introduces new copy that reads as a fresh beat.
